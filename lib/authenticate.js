    ...
    if (currentAttempts > options.noIntervalAttempts) {
      ...
      if (Date.now() - user.get(options.lastLoginField) < calculatedInterval) {
        ...
        user.save(function (saveErr) {
          ...
          const unlockAt = user.get(options.lastLoginField) + options.attemptsIntervalCalculator(newAttempts);
    ...
    if (scmp(hashBuffer, Buffer.from(user.get(options.hashField), options.encoding))) {
      ...
      if (options.limitAttempts) {
        ...
        user.save(function (saveErr) {
          ...
          const currentAttempts = user.get(options.attemptsField);
          if (currentAttempts >= options.maxAttempts) {
            return cb(null, false, new errors.TooManyAttemptsError(options.errorMessages.TooManyAttemptsError, currentAttempts));
          } else {
            return cb(null, false, new errors.IncorrectPasswordError(options.errorMessages.IncorrectPasswordError, currentAttempts));
          }
        });
      } else {
        return cb(
          null,
          false,
          new errors.IncorrectPasswordError(options.errorMessages.IncorrectPasswordError, user.get(options.attemptsField))
        );
      }
    }
  });
